...
namespace BYWGLib.Protocols
{
    ...
    public sealed class AsyncModbusTcpProtocol : IIndustrialProtocol
    {
        ...
        /// 快速诊断D4500读取问题
        /// </summary>
        public async Task<string> DiagnoseD4500IssueAsync()
        {
            var result = new System.Text.StringBuilder();
            result.AppendLine("=== D4500 诊断开始 ===");
            
            try
            {
                // 1. 测试地址解析
                result.AppendLine("1. 测试地址解析:");
                var (functionCode, startAddress, quantity) = ParseModbusAddress("D4500", "signed");
                result.AppendLine($"   D4500 -> 功能码: {functionCode}, 起始地址: {startAddress}, 数量: {quantity}");
                
                // 2. 测试其他地址格式
                result.AppendLine("2. 测试其他地址格式:");
                var (fc1, addr1, qty1) = ParseModbusAddress("4500", "signed");
                result.AppendLine($"   4500 -> 功能码: {fc1}, 起始地址: {addr1}, 数量: {qty1}");
                
                var (fc2, addr2, qty2) = ParseModbusAddress("44500", "signed");
                result.AppendLine($"   44500 -> 功能码: {fc2}, 起始地址: {addr2}, 数量: {qty2}");
                
                // 3. 测试设备连接
                result.AppendLine("3. 测试设备连接:");
                try
                {
                    var testResult = Read("40001", "uint16");
                    if (testResult != null)
                    {
                        result.AppendLine($"   40001 读取成功: {testResult}");
                    }
                    else
                    {
                        result.AppendLine("   40001 读取失败: 返回null");
                    }
                }
                catch (Exception ex)
                {
                    result.AppendLine($"   40001 读取异常: {ex.Message}");
                }
                
                // 4. 测试D4500读取
                result.AppendLine("4. 测试D4500读取:");
                try
                {
                    var d4500Result = Read("D4500", "signed");
                    if (d4500Result != null)
                    {
                        result.AppendLine($"   D4500 读取成功: {d4500Result}");
                    }
                    else
                    {
                        result.AppendLine("   D4500 读取失败: 返回null");
                    }
                }
                catch (Exception ex)
                {
                    result.AppendLine($"   D4500 读取异常: {ex.Message}");
                }
                
                // 5. 测试其他地址
                result.AppendLine("5. 测试其他地址:");
                var testAddresses = new[] { "D100", "D1000", "D2000", "D3000", "D4000" };
                foreach (var addr in testAddresses)
                {
                    try
                    {
                        var testResult = Read(addr, "signed");
                        if (testResult != null)
                        {
                            result.AppendLine($"   {addr} 读取成功: {testResult}");
                        }
                        else
                        {
                            result.AppendLine($"   {addr} 读取失败: 返回null");
                        }
                    }
                    catch (Exception ex)
                    {
                        result.AppendLine($"   {addr} 读取异常: {ex.Message}");
                    }
                }
            ...
            {
                result.AppendLine($"诊断过程中发生异常: {ex.Message}");
            }
            
            result.AppendLine("=== D4500 诊断结束 ===");
        ...
        public async Task<bool> TestDeviceConnectionAsync()
        {
            try
            {
                Log.Information("开始设备连接测试...");
                
                // 测试1：尝试读取第一个保持寄存器
                try
                {
                    var result1 = Read("40001", "uint16");
                    if (result1 != null)
                    {
                        Log.Information("设备连接测试成功，40001地址可读，值: {0}", result1);
                        return true;
                    }
                }
                catch (Exception ex)
                {
                    Log.Debug("40001地址测试失败: {0}", ex.Message);
                }
                
                // 测试2：尝试读取第一个输入寄存器
                try
                {
                    var result2 = Read("30001", "uint16");
                    if (result2 != null)
                    {
                        Log.Information("设备连接测试成功，30001地址可读，值: {0}", result2);
                        return true;
                    }
                }
                catch (Exception ex)
                {
                    Log.Debug("30001地址测试失败: {0}", ex.Message);
                }
                
                // 测试3：尝试读取D100地址
                try
                {
                    var result3 = Read("D100", "uint16");
                    if (result3 != null)
                    {
                        Log.Information("设备连接测试成功，D100地址可读，值: {0}", result3);
                        return true;
                    }
                }
                catch (Exception ex)
                {
                    Log.Debug("D100地址测试失败: {0}", ex.Message);
                }
                
                Log.Error("所有测试地址都不可用");
                return false;
            }
            catch (Exception ex)
            {
                Log.Error("设备连接测试失败: {0}", ex.Message);
                return false;
            }
        }
